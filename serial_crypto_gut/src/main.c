...
// -- state machine --
// states
enum {st_init, st_avail, st_decrypt, st_data, st_op_decrypt, st_key, st_iv, st_op_key, st_op_iv};
int state = st_init;
...
K_MSGQ_DEFINE(key_iv_msgq, KEY_IV_LEN*sizeof(uint8_t), 10, 1);
...
void processing(void *ptr1, void *ptr2, void *ptr3){
	...
	uint8_t *key = malloc(KEY_IV_LEN*sizeof(uint8_t));
	key = "BBBBBBBBBBBBBBBB"; // Default-Key
	uint8_t *iv = malloc(KEY_IV_LEN*sizeof(uint8_t));
